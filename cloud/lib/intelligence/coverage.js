...
export function buildCoverageIntelligence({spans=[],infra={},journey={},traces=[],baselines={},evidence={},events=[]}={}){
 ...
 const databaseSchema=(events||[]).some(e=>e.type==='database'&&String(e?.data?.kind||'')==='schema_table');
 const databaseIndexes=(events||[]).some(e=>e.type==='database'&&String(e?.data?.kind||'')==='index');
 const explainPlans=(events||[]).some(e=>e.type==='database'&&String(e?.data?.kind||'')==='explain_plan');
 const connectionPool=(events||[]).some(e=>(e.type==='database'&&String(e?.data?.kind||'')==='connection_pool')||(e.type==='runtime'&&/connection.*pool|pool.*connection/i.test(String(e?.data?.metric||''))));
 ...
 const signals=[
  ...
  signal('database_schema','Database schema metadata',databaseEvents?(databaseSchema?95:35):100,databaseEvents?(databaseSchema?'available':'partial'):'not_applicable',databaseSchema?'Database table/schema metadata is available':databaseEvents?'Database telemetry exists but schema metadata has not been collected':'No database adapter is reporting',['table/entity ownership and schema-aware diagnosis']),
  signal('database_indexes','Database index metadata',databaseEvents?(databaseIndexes?95:35):100,databaseEvents?(databaseIndexes?'available':'partial'):'not_applicable',databaseIndexes?'Database index metadata is available':databaseEvents?'Database telemetry exists but index metadata has not been collected':'No database adapter is reporting',['index-aware query diagnosis']),
  signal('explain_plans','Query EXPLAIN plans',databaseEvents?(explainPlans?95:30):100,databaseEvents?(explainPlans?'available':'partial'):'not_applicable',explainPlans?'Read-only EXPLAIN evidence has been collected':databaseEvents?'No EXPLAIN plan has been collected yet':'No database adapter is reporting',['planner-level query diagnosis']),
  signal('connection_pool_metrics','Connection pool visibility',connectionPool?90:databaseEvents||runtimeMetrics?35:0,connectionPool?'available':databaseEvents||runtimeMetrics?'partial':'missing',connectionPool?'Connection/pool evidence is available':'No application/database connection-pool signal has been observed',['pool exhaustion and connection contention diagnosis']),
 ...
 const weights={distributed_traces:14,service_topology:8,host_metrics:8,process_correlation:7,runtime_metrics:7,application_logs:7,error_context:4,database_visibility:5,query_visibility:5,database_schema:3,database_indexes:3,explain_plans:2,connection_pool_metrics:4,business_semantics:10,journey_correlation:10,business_value:8,change_intelligence:8,profiles:3,rum:7,network_telemetry:4,disk_performance:4,cloud_metrics:4,kubernetes_context:4};
 ...
 const categories={business:category(['business_semantics','journey_correlation','business_value','rum']),technical:category(['distributed_traces','service_topology','database_visibility','query_visibility','database_schema','database_indexes','explain_plans','connection_pool_metrics','application_logs','error_context']),runtime:category(['process_correlation','runtime_metrics','profiles']),infrastructure:category(['host_metrics','network_telemetry','disk_performance','cloud_metrics','kubernetes_context']),change:category(['change_intelligence'])};
