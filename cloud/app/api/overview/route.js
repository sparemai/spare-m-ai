...
function buildExecutive({stage,currentJourney,previousJourney,currentTechnical,previousTechnical,currentInfra,previousHosts,correlation,traces}){
 const cur=stageForView(currentJourney,stage),base=cur?previousJourney?.stages?.find(s=>s.name===cur.name):null;
 const top=currentTechnical?.top_operations?.[0]||null;
 ...
 const confidence=Math.min(95,Math.round(Number(currentJourney?.confidence||50)*.55+(distributed?10:0)+(hasPrior?10:0)+(hasInfra?10:0)+(hasCorr?8:0)));
 const stageName=cur?.name||(stage==='All'?'Booking':stage);
 const technicalLabel=top?`${top.service} • ${top.operation}`:'No technical contributor isolated';
 ...
 const summary=status==='HEALTHY'?`No material degradation is visible in the selected ${stageName} context.`:`${stageName} shows ${cur?.p95_ms?`p95 ${Math.round(cur.p95_ms)} ms`:''}${cur?.error_rate?` and ${cur.error_rate}% errors`:''}. The strongest sampled technical signal is ${technicalLabel}. ${infraText}`;
 return {
  status,headline,summary,confidence,
  business:{journey:'Booking',stage:stageName,p95_ms:Number(cur?.p95_ms||0),p95_delta_pct:appDelta,error_rate:Number(cur?.error_rate||0),error_trace_rate:errorTraceRate,selected_traces:traces.length,error_traces:errorTraces,baseline_p95_ms:Number(base?.p95_ms||0)||null,conversion_available:false,revenue_available:false},
  technical:{service:top?.service||null,operation:top?.operation||null,p95_ms:Number(top?.p95_ms||0),p95_delta_pct:opDelta,error_rate:Number(top?.error_rate||0),baseline_p95_ms:Number(topBase?.p95_ms||0)||null},
  ...
  path:[{type:'business',label:'Booking journey'},{type:'stage',label:stageName},{type:'service',label:top?.service||'No service isolated'},{type:'operation',label:top?.operation||'No operation isolated'},{type:'infra',label:`Infrastructure ${infraLikelihood}`}],
  data_gaps:[...(!currentJourney?.mapping||currentJourney.mode==='business-semantic'?[]:['True user conversion requires sparem.business.step + journey/session correlation id.']),...['Revenue-at-risk requires a business value/order amount attribute.']]
 };
}
...
export async function GET(req){
 try{
  ...
  if(!agent)return Response.json({agent_id:null,filters:{range:rangeKey,journey:'Booking',stage:selectedStage},executive:null,journey:{stages:[]},technical:{edges:[],top_operations:[],service_stats:[]},infra:{current:null,baseline:null},correlation:{points:[]},evidence:{observations:['Waiting for telemetry.'],actions:[]},recent_traces:[]});
