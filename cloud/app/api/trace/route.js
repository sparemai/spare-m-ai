...
export async function GET(req){
 try{
  ...
  return Response.json({trace_id:id,summary:{duration_ms:intelligence?.duration_ms||0,span_count:rows.length,service_count:services.length,services,error_count:errors,root_operation:intelligence?.root_operation||null,slowest_span:rows.reduce((a,b)=>Number(b.duration_ms||0)>Number(a?.duration_ms||0)?{span_id:b.span_id,service:b.service,operation:b.operation,duration_ms:Number(b.duration_ms||0)}:a,null)},intelligence,spans:rows});
