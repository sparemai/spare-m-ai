...
export async function POST(req){
 try{
  ...
  if(!process.env.OPENAI_API_KEY)return Response.json({agent_id:agent,range:rangeKey,stage,agentic:false,reason:'OPENAI_API_KEY not configured',deterministic_assessment:intel.evidence,capabilities:intel.capabilities});
  ...
  let input=[{role:'user',content:`Investigate the current SPARE-M reliability state for business journey Booking, stage focus ${stage}, time window ${rangeKey}. Use tools to gather evidence before concluding. Do not assume root cause. Distinguish observed fact, correlation, inference and confirmed cause.`}];
  const used=[];let response;
  for(let iteration=0;iteration<7;iteration++){
   response=await client.responses.create({model,store:false,reasoning:{effort:body.mode==='deep'?'medium':'low'},text:{verbosity:'low',format},instructions:'You are the SPARE-M autonomous reliability investigator. Business impact first, then trace/service evidence, then infrastructure. Use deterministic tool outputs as facts. Never call an inferred journey a confirmed customer transaction when linkage confidence is weak. Prefer exclusive/self-time and baseline deviation over raw inclusive span duration for contributor ranking. Correlation is not causation. Challenge your leading hypothesis with contrary evidence. If evidence is insufficient, say so and request the smallest additional telemetry needed.',tools,tool_choice:'auto',input});
  ...
  return Response.json({agent_id:agent,range:rangeKey,stage,agentic:true,model,tools_used:[...new Set(used)],response:result,deterministic_leading_hypothesis:intel.evidence.leading_hypothesis,capabilities:intel.capabilities});
