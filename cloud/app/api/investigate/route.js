...
export async function POST(req){
 try{
  ...
  for(let iteration=0;iteration<7;iteration++){
   response=await client.responses.create({model,store:false,include:['reasoning.encrypted_content'],reasoning:{effort:body.mode==='deep'?'medium':'low'},text:{verbosity:'low',format},instructions:'You are the SPARE-M autonomous reliability investigator. Business impact first, then trace/service evidence, then infrastructure. Use deterministic tool outputs as facts. Never call an inferred journey a confirmed customer transaction when linkage confidence is weak. Prefer exclusive/self-time and baseline deviation over raw inclusive span duration for contributor ranking. Correlation is not causation. Challenge your leading hypothesis with contrary evidence. If evidence is insufficient, say so and request the smallest additional telemetry needed.',tools,tool_choice:'auto',input});
