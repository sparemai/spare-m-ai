...
function Header({data,range,setRange,stage,setStage,refresh,loading}){
 return <><header className="nav"><div className="brand"><div className="brandMark">M</div><div><b>SPARE-M</b><span>Business Reliability</span></div></div><div className="navRight"><span className="liveDot"/>Live telemetry <b>{data?.agent_id||'—'}</b><button onClick={refresh} disabled={loading}>{loading?'Refreshing…':'Refresh'}</button></div></header><div className="toolbar"><div><span>Application</span><b>EasyTravel</b></div><label>Time<select value={range} onChange={e=>setRange(e.target.value)}>{RANGES.map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></label><label>Focus<select value={stage} onChange={e=>setStage(e.target.value)}>{STAGES.map(v=><option key={v}>{v}</option>)}</select></label><div className="windowText"><span>Showing</span><b>{when(data?.filters?.from)} → {when(data?.filters?.to)}</b></div></div></>
}
...
function KPIs({data}){
 ...
 return <section className="kpiGrid">{cards.map(([label,value,sub,t])=><article className={`kpi ${t}`} key={label}><span>{label}</span><b>{value}</b><small>{sub}</small></article>)}</section>
}

function BusinessFlow({data}){
 const b=data?.business_context||{},stages=b.stages||[];
 return <section className="card"><div className="sectionTitle"><div><span>BUSINESS VIEW</span><h2>Booking flow</h2><p>How customers are moving through the travel booking journey in the selected period.</p></div><Pill>{b.measurement==='sampled_trace_proxy'?'Trace-observed activity':'Business events'}</Pill></div><div className="flow">{stages.map((s,i)=><div className="flowWrap" key={s.name}><div className={`flowStep ${s.error_rate>2?'problem':''}`}><span>{s.name}</span><b>{s.trace_count}</b><small>observed journeys</small><div><em>p95 {ms(s.p95_ms)}</em><em>{pct(s.error_rate)} errors</em></div></div>{i<stages.length-1&&<i className="arrow">→</i>}</div>)}</div><div className="plainNote">{b.note}</div></section>
}
...
function TraceDrawer({trace,onClose}){
 ...
 const spans=trace.spans||[],ti=trace.intelligence||{},sum=trace.summary||{},byId=new Map(spans.map(s=>[s.span_id,s]));
